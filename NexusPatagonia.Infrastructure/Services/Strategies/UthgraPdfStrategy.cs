...
namespace NexusPatagonia.Infrastructure.Services.Strategies
{
    public class UthgraPdfStrategy : IPdfProcessingStrategy
    {
        ...
        public async Task<IExtractedData> ProcessAsync(Stream pdfStream)
        {
            ...
            {
                // We process the first page as it contains the main header
                var headerPage = document.GetPage(1);
                var fullContent = string.Join(" ", headerPage.GetWords().Select(w => w.Text));

                // Regex to capture the Name and CUIT from the "Razón Social" field
                // Example in PDF: "Razón Social: POPULAR PATAGONIA SAS-30718750845"
                string businessPattern = @"Razón Social:\s*([^-\n]+)\s*-\s*(\d{11})";

                var match = Regex.Match(fullContent, businessPattern);

                if (match.Success)
                { 
                    results.CompanyName = match.Groups[1].Value.Trim();
                    results.Cuit = match.Groups[2].Value.Trim();
                }
